---
description: Rename the boilerplate's display identity (title, package name, app name, portal titles) to a real project's — display text only.
argument-hint: '<project name> — <one-sentence description> [— portal names, if renaming]'
allowed-tools: Read, Edit, Bash(pnpm run *), Bash(pnpm exec *)
---
