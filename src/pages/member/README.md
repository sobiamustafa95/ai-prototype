...
The `ROLES.MEMBER`-only pages — registered in `src/routes/ProtectedRoutes.tsx`'s
`MEMBER_PROTECTED_ROUTES` (`roles: [ROLES.MEMBER]`, plus `nav` for its sidebar entry — one
