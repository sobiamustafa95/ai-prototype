...
- `VITE_API_BASE_URL` — host for the Axios client (`src/services/api-client.ts`); empty =
  same-origin (dev + tests use MSW). Route paths in `src/constants/api-routes.ts` are already
  absolute (`/auth/login`), so this is host-only.
...
| `components/<role>/`  | Components scoped to one role's own pages (mirrors `pages/<role>/`). Empty by default — both shipped roles (`member`, `admin`) currently need no role-specific components beyond their one placeholder page.                           |
...
| `pages/common/`       | Pages every role can reach: `HomePage`, `ForbiddenPage`, `ExamplePage`, and the shipped common-route placeholders (`SettingsPage`, `ProfilePage`, `NotificationsPage`).                                                                |
| `pages/auth/`         | Route-level glue for the 5 auth pages — thin wrappers around `components/auth/`'s forms.                                                                                                                                               |
| `pages/<role>/`       | Pages owned by one role. Ships with `member/DashboardPage.tsx` (`ROLES.MEMBER`) and `admin/AdminDashboardPage.tsx` (`ROLES.ADMIN`) — both placeholders, not real features.                                                             |
| `hooks/`              | Generic, reusable hooks: `useDebouncedValue`, `useThemeSync`, `useSyncAuthAcrossTabs`. Feature-specific hooks (e.g. `useExampleItems`) live inside their own feature folder instead.                                                   |
| `lib/`                | `utils.ts` — the `cn()` class-merge helper every CVA/Radix component uses.                                                                                                                                                             |
| `services/`           | `api-client.ts` (Axios instance: bearer-token attach + single-flight refresh-and-retry on 401), `authService.ts` (the real `/auth` contract), `queryClient.ts` (the app's one TanStack `QueryClient`, with global error-toast wiring). |
| `utils/`              | Pure, side-effect-free functions only — 100% unit-testable by construction.                                                                                                                                                            |
| `schemas/`            | Zod schemas: `common.schema.ts` (shared primitives — email/password/phone/url), `auth.schema.ts`, `example.schema.ts`. Form schemas and API-contract schemas both live here.                                                           |
| `types/`              | Shared TS `interface`s. The one folder where barrel `index.ts` re-exports are allowed (never for components).                                                                                                                          |
| `constants/`          | `api-routes.ts` (every API path), `config.ts`, `env.ts` (the Zod-validated `import.meta.env` wrapper — see § 1).                                                                                                                       |
...
- **`components/example/` + `pages/common/ExamplePage.tsx`** — a generic, data-driven,
  ...
  (`useExampleItems.ts`) with a co-located query-key factory, a feature-scoped Zustand store
  (`exampleWidgetStore.ts`), full accessibility, and its `*.workflow.test.tsx`. Copy this shape
  for any new **list/browse/search** feature.
...
3. **Wire it into routing** (if it's a new page) — see § 2's routing walkthrough; a role-gated
   page goes through `ProtectedRoutes.tsx`, a guest-only page through `PublicRoutes.tsx`. Every
   page > 50 KB gets `lazy()`-imported next to its own route entry.
4. **Data fetching** — a TanStack Query hook, never a raw `useEffect` fetch. Co-locate a query
   key factory with the hook (never a global `queryKeys.ts`):
   ```ts
...
### 4b. `pnpm verify` — the canonical, exhaustive gate (15 checks, always full-repo)
...
| 6   | `check:types`              | `tsc -b --noEmit`, strict (`exactOptionalPropertyTypes`, `noPropertyAccessFromIndexSignature`, full strict family, on both `tsconfig.app.json` and `tsconfig.node.json`)                                                                                                            | Yes                   |
| 7   | `check:test`               | `vitest run` — placed right after types, before lint/format/style, so a broken component fails fast before the expensive build stage                                                                                                                                                | Yes                   |
| 8   | `check:lint`               | `eslint . --max-warnings=0` (`tseslint.configs.strictTypeChecked`, hooks, hardcoded-text, token rules)                                                                                                                                                                              | Yes                   |
| 9   | `check:a11y`               | Strict `jsx-a11y` pass (`eslint.a11y.config.js`)                                                                                                                                                                                                                                    | Yes                   |
| 10  | `check:format`             | `prettier --check`                                                                                                                                                                                                                                                                  | Yes                   |
| 11  | `check:style`              | `impeccable detect src/ < /dev/null`                                                                                                                                                                                                                                                | Yes                   |
| 12  | `check:doctor`             | `react-doctor src/ --no-supply-chain --yes < /dev/null` — **only `error`-severity findings fail this**; `warning`-severity findings (e.g. the documented `query-mutation-missing-invalidation` exceptions on `signup`/`forgotPassword`) show up in output but do not fail the check | Only `error` severity |
| 13  | `check:build`              | `pnpm build` (`tsc -b && vite build`, production mode)                                                                                                                                                                                                                              | Yes                   |
| 14  | `check:build-budget`       | `size-limit` against the real `dist/assets/` output from step 13 — entry bundle ≤ 195 kB gzip, total JS ≤ 268 kB gzip (`.size-limit.js`)                                                                                                                                            | Yes                   |
| 15  | `check:e2e`                | `playwright test` against a fresh `pnpm build:e2e` + `vite preview` — deliberately last, since it needs a real build and is the single most expensive check                                                                                                                         | Yes                   |
